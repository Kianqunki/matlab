...
for k=1:timesteps-1
    ...
    % z is a struct with 'measurement' field (=relative position) and 
    % 'descriptor' field (= landmark ID)
    % descriptors is a global variable that keeps track of discovered
    % landmarks
    % NOTE: data association is implemented in another version
    [particles, descriptors] = fastslam_update_2d_SLAM(particles, z(k+1), R, descriptors,X_L);
